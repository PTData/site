...
Over the years, as a way to keep myself up to date and to improve my skills, I attended and completed several courses in schools like LxSchool (php , MySQL) or Altalógica ( PHP OOP , Java ), and also acquired basic knowledge on C++ and Java as an autodidact . My interest on information technology encourages me to search for new challenges.  ";
    ...
    {
        $todo = [
            //'lista de musica spotify ou assim',
            //'icons footer: linkedIn, facebook, twitter, strava e github!!',
            //'pequenas experi|encias',
            //'exportação odf curriculum'
        ] ;

        return $this->render('site/about/about.html.twig', [
            'about' => $this->aboutText(), 'todo' => $todo
        ]);
    }

    public function cv(Request $request, EntityManagerInterface $entityManager): Response
    {
        $service = new CurriculumService($entityManager);
        ;
        return $this->render('site/curriculum/curriculum.html.twig', [
            'content' => $service->get()
        ]);
    }
